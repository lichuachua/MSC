...
def add_labels(image, x_labels, y_labels, label_width=150, label_height=50):
    # 创建一个额外的空间以添加纵坐标标签
    ...
    for i, label in enumerate(x_labels):
        draw.text((label_width + i * (image.width // len(x_labels)), label_height // 2), label, fill="black", font=font)

    # 添加纵坐标标签
    for i, label in enumerate(y_labels):
        draw.text((10, label_height + i * (image.height // len(y_labels)) + 10), label, fill="black", font=font)
...
def main(input_folder, output_file):
    ...
    # 添加坐标标签
    final_image_with_labels = add_labels(final_image, x_labels, y_labels)
